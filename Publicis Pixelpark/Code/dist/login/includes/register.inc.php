...
if (isset($_POST['username'], $_POST['email'], $_POST['p'])) {
    ...
    if (empty($error_msg)) {
		...
		$_db_passwort = "{ _db_passwort }";
		$_db_datenbank = "{ _db_datenbank }";
		...
		$_sql = "SELECT id FROM members WHERE username = '" . $_POST['username'] . "'";
		$_resultat = mysql_query($_sql);
		if (!$_resultat) { // add this check.
			die('Invalid query: ' . mysql_error());
		}
		$_daten = mysql_fetch_array($_resultat);
		mysql_close($_link);
        if ($insert_stmt2 = $mysqli->prepare("INSERT INTO profil (user_id, username) VALUES (?, ?)")) {
            $insert_stmt2->bind_param('ss', $_daten[0], $username);
            // Führe die vorbereitete Anfrage aus.
